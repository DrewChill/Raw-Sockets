...
#define HAS_TCP_OPTIONS(ptr) (ptr->th_off > TCP_WORD_LENGTH_WITH_NO_OPTIONS)
#define TCP_OPTION_OFFSET(ptr) ((char*)ptr + (TCP_WORD_LENGTH_WITH_NO_OPTIONS * WORD_LENGTH))
#define TCP_OPTION_LENGTH(ptr) ((ptr->th_off - TCP_WORD_LENGTH_WITH_NO_OPTIONS) * WORD_LENGTH)
#define END_OF_TCP_OPTION_CHECK(ptr) ((*ptr) == 0)
#define TCP_OPTIONS_LEN(ptr) ((ptr->th_off - TCP_WORD_LENGTH_WITH_NO_OPTIONS) * WORD_LENGTH )
...
#define IS_DUPLICATE_TCP_SEGMENT(tcph) (ntohl(tcph->th_seq) < tcp_state.server_next_seq_num)
#define IS_DUPLICATE_ACK(tcph) ((tcph->th_flags & TH_ACK) && (tcph->th_ack == tcp_state.last_acked_seq_num) )
                                   ...
                                   uint16_t tcp_payload_len, uint32_t src_addr, uint32_t dst_addr)
{
    pseudo_header psh;
    char* pseudogram;
    uint16_t tcphdr_len = (tcph->th_off * WORD_LENGTH);
    ...
    tcph->th_sum = csum((uint16_t*) pseudogram, (unsigned int) psize);
    free(pseudogram);
}

static int validate_ip_checksum(struct ip* iph)
{
    int ret = -1;
    uint16_t received_checksum = iph->ip_sum;
    iph->ip_sum = 0;
    
    if (received_checksum
        == csum((uint16_t*) iph, (unsigned int) (iph->ip_hl * WORD_LENGTH)))
        ret = 1;
...
{
    int ret = -1;
    uint16_t received_checksum = tcph->th_sum;
    tcph->th_sum = 0;
    ...
    if (received_checksum == tcph->th_sum)
        ret = 1;
    return ret;
}
...
static packet_t* create_packet()
{
    ...
    packet->offset[TCP_OFFSET] = packet->payload + sizeof(struct ip);
...
static void adjust_layer_offset(packet_t* packet)
{
    ...
    iph = (struct ip *) packet->payload;
    tcph = (struct tcphdr *) (packet->payload + (iph->ip_hl * WORD_LENGTH));
    packet->offset[TCP_OFFSET] = (char*) tcph;
    packet->offset[DATA_OFFSET] = (char*) (packet->offset[TCP_OFFSET]
                                           + (tcph->th_off * WORD_LENGTH));
}
...
{
    iph->ip_dst.s_addr = *(uint32_t*) &tcp_state.session_info.dst_addr.sin_addr.s_addr;
    iph->ip_src.s_addr = *(uint32_t*) &tcp_state.session_info.src_addr.sin_addr.s_addr;
    iph->ip_hl = 5;
    iph->ip_p = IPPROTO_TCP;
    iph->ip_ttl = 255;
    iph->ip_v = 4;
    iph->ip_len = sizeof(struct ip) + ip_payload_len;
...
{
    tcph->th_dport = *(uint16_t*) &tcp_state.session_info.dst_addr.sin_port;
    tcph->th_sport = *(uint16_t*) &tcp_state.session_info.src_addr.sin_port;
    tcph->th_win = htons(tcp_state.client_window_size);
    tcph->th_seq = htonl(tcp_state.client_next_seq_num);
    tcp_state.client_next_seq_num +=
    (flags->syn || flags->fin) ? 1 : payload_len;
    tcph->th_off = (flags->syn) ? 6 : 5;
                                 ...
                                 tcp_flags_t* flags)
{
    ...
    calculate_tcp_checksum(tcph, payload_len,
                           *(uint32_t *) &tcp_state.session_info.src_addr.sin_addr.s_addr,
                           *(uint32_t *) &tcp_state.session_info.dst_addr.sin_addr.s_addr);
    build_ip_header(iph, ((tcph->th_off * WORD_LENGTH) + payload_len));
}
...
static int send_ack_segment(uint8_t fin)
{
    ...
    if ((ret = send_packet(&packet->payload,
                           ((struct ip*) packet->offset[IP_OFFSET])->ip_len)) < 0)
    {
...
static int receive_packet(packet_t *packet)
{
    ...
    while (1)
    {
        ...
        struct ip *iph = (struct ip *) &packet->payload;
        ...
        uint16_t iphdr_len = iph->ip_hl * WORD_LENGTH;
        struct tcphdr *tcph = (struct tcphdr *) ((char*) iph + iphdr_len);
        uint16_t tcphdr_len = tcph->th_off * WORD_LENGTH;
        
        if (iph->ip_src.s_addr != tcp_state.session_info.dst_addr.sin_addr.s_addr
            && tcph->th_dport != tcp_state.session_info.src_port
            && tcph->th_sport != tcp_state.session_info.dst_port)
            continue;
        
        if (validate_tcp_checksum(tcph,
                                  (ntohs(iph->ip_len) - iphdr_len - tcphdr_len)) < 0)
        {
        ...
        packet->payload_len = (ntohs(iph->ip_len) - iphdr_len - tcphdr_len);
        break;
    }
    return ret;
}
...
{
    tcp_state.server_next_seq_num = (ntohl(tcph->th_seq) + payload_len);
    tcp_state.last_acked_seq_num = (ntohl(tcph->th_ack));
    
    pthread_mutex_lock(&tcp_state.tcp_state_lock);
    tcp_state.server_window_size = ntohs(tcph->th_win);
    ...
    remove_acked_entries(ntohl(tcph->th_ack));
...
static void retransmission_timer_handler(uint16_t buffer_index)
{
...
dispatch_source_t create_retransmission_timer(int send_buffer_index, double interval, dispatch_queue_t queue, dispatch_block_t retransmitBlock)
{
    dispatch_source_t timer = dispatch_source_create(DISPATCH_SOURCE_TYPE_TIMER, 0, 0, queue);
    
    if(timer){
        dispatch_source_set_timer(timer, dispatch_time(DISPATCH_TIME_NOW, interval*NSEC_PER_SEC), interval*NSEC_PER_SEC, (1ull * NSEC_PER_SEC) / 10);
        dispatch_source_set_event_handler(timer, retransmitBlock);
...
static int send_tcp_segment(packet_t* packet)
{
    ...
    double retransmitTime = 60.000f;
    
    packet->retx_buffer_index = tcp_state.sender_info.retx_buffer_tail;
    
    packet->timer = create_retransmission_timer(tcp_state.sender_info.retx_buffer_tail, retransmitTime, queue, ^{
        retransmission_timer_handler(packet->retx_buffer_index);
    });
    
    pthread_mutex_lock(&tcp_state.sender_info.tcp_retx_lock);
    
    tcp_state.sender_info.retx_buffer[tcp_state.sender_info.retx_buffer_tail].packet_seq =
    ((struct tcphdr*) &packet->offset[TCP_OFFSET])->th_seq;
...
static int receive_syn_ack_segment(tcp_flags_t* flags)
{
    ...
    while (1)
    {
        ...
        if ((tcph->th_flags & TH_ACK) == flags->ack && (tcph->th_flags & TH_SYN) == flags->syn)
            break;
...
static void* tcp_recv_handler(void* args)
{
    ...
    while (1)
    {
        ...
        tcph = (struct tcphdr*) packet->offset[TCP_OFFSET];
        iph = (struct ip*) packet->offset[IP_OFFSET];
        
        if (ntohl(tcph->th_seq) != (tcp_state.server_next_seq_num))
        {
            send_ack_segment(0);
        ...
        uint16_t payload_len = ntohs(iph->ip_len) - (iph->ip_hl * WORD_LENGTH)
        - (tcph->th_off * WORD_LENGTH);
        ...
        if ((tcph->th_flags & TH_FIN) && (tcp_state.tcp_current_state & ESTABLISHED))
        {
            pthread_mutex_lock(&tcp_state.tcp_state_lock);
            process_ack(tcph, 1);
            send_ack_segment(0);
        ...
        else if ((tcph->th_flags & TH_FIN) && (tcph->th_flags & TH_ACK)
        ...
        else if ((tcph->th_flags & TH_FIN) && (tcp_state.tcp_current_state & FIN_WAIT_1))
        {
            pthread_mutex_lock(&tcp_state.tcp_state_lock);
            process_ack(tcph, 1);
            send_ack_segment(0);
        ...
        else if ((tcph->th_flags & TH_FIN) && (tcp_state.tcp_current_state & FIN_WAIT_2))
        {
            pthread_mutex_lock(&tcp_state.tcp_state_lock);
            process_ack(tcph, 1);
            send_ack_segment(0);
